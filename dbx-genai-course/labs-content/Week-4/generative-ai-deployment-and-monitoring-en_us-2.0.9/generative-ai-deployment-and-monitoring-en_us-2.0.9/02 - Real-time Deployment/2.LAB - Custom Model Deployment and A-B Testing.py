...
try:
    ...
    # Fallback for serverless compute where cluster tags are not available
    schemas = [row.databaseName for row in spark.sql("SHOW SCHEMAS IN genai_shared_catalog_04").collect() if row.databaseName.startswith("ws_")]
    shared_schema_name = schemas[0]
...
try:
    ...
    db_host = w.config.host.rstrip("/").replace("https://", "").replace("http://", "")
