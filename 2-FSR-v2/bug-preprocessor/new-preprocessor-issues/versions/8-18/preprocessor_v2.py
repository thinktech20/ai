...
class FSRV2Preprocessor:
    ...
    def _collect_heading_candidates(
        ...
        for match in re.finditer(SUBSEC_GENERIC, full_text):
            ...
            # Non-Generator flip-back keywords: these indicate a return to GT context
            # and must be allowed through even when the preceding context was Generator.
            _gt_flipback = re.match(
                r'^(rotor|turbine|compressor|combustion|inlet|hot\s+gas|exhaust|bearing)\b', title
            )
            if not context_type or (context_type == "Generator" and not _gt_flipback):
                continue
