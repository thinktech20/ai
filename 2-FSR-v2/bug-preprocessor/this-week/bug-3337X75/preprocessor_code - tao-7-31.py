...
def preprocess(ctx):
    ...
    for pat in ST_LABELS:
        for m in re.finditer(pat, title, re.I):
            set_type(m.group(1), 'Steam Turbine')
    ...
    st_esn = first_active('Steam Turbine')
    ...
    for e in sorted(active):
        if esn_type.get(e) == 'Gas Turbine' and not gt_esn:
            ...
            gen_esn = e
        elif esn_type.get(e) == 'Steam Turbine' and not st_esn:
            st_esn = e
    ...
    for esn_candidate, cnt in _esn_counts.items():
        if cnt >= 3 and len(esn_candidate) >= 6:
            ...
            # FIX 9b: Register broad-scanned ESNs so they can be assigned to regions.
            if esn_candidate not in esn_type:
                inferred = 'Generator' if re.search(r'[A-Z]', esn_candidate) else 'Gas Turbine'
                set_type(esn_candidate, inferred)
                active.add(esn_candidate)
    ...
    if len(distinct_types) >= 2:
        ...
        for i, ptext in enumerate(pages):
            ...
            if is_st and st_esn:
                ...
                # FIX 1+9c: Pick the most-mentioned active ESN on this page.
                page_esn_counts = {}
                for e in active:
                    c = ptext.count(e)
                    if c > 0:
                        page_esn_counts[e] = c
                found_esn = max(page_esn_counts, key=page_esn_counts.get) if page_esn_counts else None
                if found_esn:
                    if found_esn == st_esn:
                        page_assign[i] = ('Steam Turbine', st_esn)
                    else:
