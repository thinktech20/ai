...
# FIX 5: Expanded ESN capture to include GG-format (2 letters + 4-6 digits)
...
GEN_LABELS = [
    r'Gen(?:erator)?\s*(?:Field\s*)?(?:ESN|Serial\s*(?:No|#|Number))\.?\s*[:#]\s*(\d{3}[A-Z]\d{3})',  # FIX 3: (?:Field\s*)? added
    r'Generator\s*#\s*(\d{3}[A-Z]\d{3})',  # FIX 4: "Generator # 338X233" pattern
    r'(?:Equipment\s+)?Generator\s+(?:No|Number)\.?\s*[:#]?\s*(\d{3}[A-Z]\d{3})',  # FIX 10: "Equipment Generator No.: 337X765"
    r'(GG\d{4,6})\s*\|\s*SY\d{7}',  # FIX 5: GG-format ESN on title page
]
# FIX 6: Steam Turbine ESN detection (st_esn, ST_LABELS, boundaries, page fallback)
ST_LABELS = [
...
def _choose_type_anchor_esn(esn_type, active_esns, equip_type, ordered_candidates=None, fallback_rank=None):
    """Choose a stable ESN anchor for an equipment type using observed evidence before fallback."""
    same_type = [e for e in active_esns if esn_type.get(e) == equip_type]
    if not same_type:
        return None
    if len(same_type) == 1:
        return same_type[0]

    # Prefer candidates seen earliest from stronger context (title / explicit boundaries).
    if ordered_candidates:
        for esn in ordered_candidates:
            if esn in same_type:
                return esn

    # Prefer most frequently observed ESN in structured boundary matches.
    if fallback_rank:
        ranked = sorted(
            same_type,
            key=lambda e: (
                -fallback_rank.get(e, {}).get('count', 0),
                fallback_rank.get(e, {}).get('first_pos', 10**12),
                e,
            ),
        )
        if ranked:
...
def preprocess(ctx):
    ...
    esn_type = {}
    header_hits = []
    title_candidates_by_type = {
        'Gas Turbine': [],
        'Generator': [],
        'Steam Turbine': [],
    }

    def _push_title_candidate(equip_type, esn):
        if equip_type not in title_candidates_by_type:
            return
        n = _norm(esn)
        if n and n not in title_candidates_by_type[equip_type]:
            title_candidates_by_type[equip_type].append(n)
    ...
    for i, ptext in enumerate(pages):
        for m in re.finditer(HEADER, ptext, re.I):
            ...
            set_type(esn, t)
            if i < 6:
                _push_title_candidate(t, esn)

    for pat in GT_LABELS:
        for m in re.finditer(pat, title, re.I):
            esn = _norm(m.group(1))
            set_type(esn, 'Gas Turbine')
            _push_title_candidate('Gas Turbine', esn)
    for pat in GEN_LABELS:
        for m in re.finditer(pat, title, re.I):
            esn = _norm(m.group(1))
            set_type(esn, 'Generator')
            _push_title_candidate('Generator', esn)
    for pat in ST_LABELS:
        for m in re.finditer(pat, title, re.I):
            esn = _norm(m.group(1))
            set_type(esn, 'Steam Turbine')
            _push_title_candidate('Steam Turbine', esn)
    ...
    gt_esn = _choose_type_anchor_esn(esn_type, active, 'Gas Turbine', title_candidates_by_type['Gas Turbine'])
    gen_esn = _choose_type_anchor_esn(esn_type, active, 'Generator', title_candidates_by_type['Generator'])
    st_esn = _choose_type_anchor_esn(esn_type, active, 'Steam Turbine', title_candidates_by_type['Steam Turbine'])
    ...
    # Re-derive gen_esn/gt_esn/st_esn with newly discovered ESNs.
    # Use title/boundary evidence before deterministic lexical fallback.
    active = (all_esns | set(esn_type.keys())) - inactive
    boundary_rank_by_type = {
        'Gas Turbine': {},
        'Generator': {},
        'Steam Turbine': {},
    }
    for m in re.finditer(HEADER, full, re.I):
        t = m.group(1).title()
        e = _norm(m.group(2))
        if t not in boundary_rank_by_type:
            continue
        entry = boundary_rank_by_type[t].setdefault(e, {'count': 0, 'first_pos': m.start()})
        entry['count'] += 1
        if m.start() < entry['first_pos']:
            entry['first_pos'] = m.start()

    gt_esn = _choose_type_anchor_esn(
        esn_type,
        active,
        'Gas Turbine',
        title_candidates_by_type['Gas Turbine'],
        boundary_rank_by_type['Gas Turbine'],
    )
    gen_esn = _choose_type_anchor_esn(
        esn_type,
        active,
        'Generator',
        title_candidates_by_type['Generator'],
        boundary_rank_by_type['Generator'],
    )
    st_esn = _choose_type_anchor_esn(
        esn_type,
        active,
        'Steam Turbine',
        title_candidates_by_type['Steam Turbine'],
        boundary_rank_by_type['Steam Turbine'],
    )
    ...
    # FIX 9: Broad ESN scan — find ALL ESN-shaped strings in full text.
    # Captures ESNs that appear without structured labels (e.g. in prose,
    # filenames, footers). Requires 3+ occurrences to filter noise.
    _esn_counts = {}
    for m in re.findall(r'\b(\d{3}[A-Z]\d{3})\b', full, re.I):
        k = m.upper()
        _esn_counts[k] = _esn_counts.get(k, 0) + 1
    # Also count numeric-only ESN pattern (e.g. Steam Turbine 155360)
    for m in re.findall(r'\b(\d{6})\b', full):
        if m in esn_type:
            _esn_counts[m] = _esn_counts.get(m, 0) + 1
    broad_esns = set(esn_type.keys())  # start with label-detected
    for esn_candidate, cnt in _esn_counts.items():
        if cnt >= 3 and len(esn_candidate) >= 6:
            broad_esns.add(esn_candidate)
    ...
    for m in re.finditer(SECTION_HDR, full):
        t = _canon(m.group(1))
        e = gen_esn if t in ('Generator', 'Exciter') else (gt_esn if t == 'Gas Turbine' else (st_esn if t == 'Steam Turbine' else None))
    ...
    # FIX 7: (?:#{1,3}\s+)? handles DS Guru's _inject_pdf_heading_markers which prepends ## / ###
    ...
    # FIX 8: Filter out TOC-area matches (first 5% of doc) + order-independent boundaries
    _toc_cutoff = len(full) // 20
    gen_subsecs = [(p, n) for p, n in gen_subsecs if p > _toc_cutoff]
    gt_subsecs = [(p, n) for p, n in gt_subsecs if p > _toc_cutoff]
    # Add Generator subsection boundaries
    if gen_subsecs and gen_esn:
        for gs_pos, gs_num in gen_subsecs:
            boundaries.append((gs_pos, 'Generator', gen_esn))
    # Add Turbine/ST subsection boundaries (order-independent)
    if gen_subsecs and gt_subsecs and (gt_esn or st_esn):
        turbine_type = 'Gas Turbine' if gt_esn else 'Steam Turbine'
        turbine_esn = gt_esn or st_esn
        for gt_pos, gt_num in gt_subsecs:
            boundaries.append((gt_pos, turbine_type, turbine_esn))
    ...
    if len(distinct_types) >= 2:
        ...
        for i, ptext in enumerate(pages):
            ...
            # FIX 6: ST page-level fallback — ESN-presence only (no ST_SIGNATURES needed)
            is_st = st_esn and st_esn in ptext and not (gen_esn and gen_esn in ptext)
            if is_st and st_esn:
                page_assign[i] = ('Steam Turbine', st_esn)
            elif is_gen and not is_gt and gen_esn:
                ...
                if found_esn:
                    if found_esn == st_esn:
                        page_assign[i] = ('Steam Turbine', st_esn)
                    elif primary_type and primary_type != 'shared':
    ...
    if 'gen_esn' in field_names and gen_esn:
        doc['gen_esn'] = gen_esn
    if 'st_esn' in field_names and st_esn:
        doc['st_esn'] = st_esn
    if 'all_esns' in field_names and broad_esns:
        doc['all_esns'] = ", ".join(sorted(broad_esns))
    ...
    lines.append(
        ...
        + f" (gt_bytes={gt_bytes}, gen_bytes={gen_bytes}, st_bytes={st_bytes}, "
